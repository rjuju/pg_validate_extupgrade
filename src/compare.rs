 ...
 *---------------------------------------------------------------------------*/
use crate::pgdiff::{DiffSource, SchemaDiff};
...
pub fn compare_map<'a, T>(
    self_map: &'a BTreeMap<String, T>,
    other_map: &'a BTreeMap<String, T>,
    typname: &'static str,
        ...
        match other_map.get(ident) {
            ...
            Some(o) => {
                if let Some(f) = fn_comp {
                    f(ident, self_map.get(ident).unwrap(), o, &mut diffs);
                }
            }
        }
    }
...
impl<'a, T: Compare<'a>> Compare<'a> for Option<T> {
    fn compare(&'a self, other: &'a Option<T>) -> Option<SchemaDiff<'a>> {
        if self.is_none() && !other.is_none() {
            return Some(SchemaDiff::NoneDiff(DiffSource::Installed, other.value()));
        }

        if !self.is_none() && other.is_none() {
            return Some(SchemaDiff::NoneDiff(DiffSource::Upgraded, self.value()));
        }
