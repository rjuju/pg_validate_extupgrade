...
impl<'a> Compare<'a> for ExecutedQueries {
    fn compare(&'a self, other: &'a Self) -> Option<SchemaDiff<'a>> {
        compare_map(&self.queries, &other.queries, "Resultset", Some(query_cmp))
    }
}
